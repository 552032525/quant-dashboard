      ...
      <div class="bg-[#0f1a2e] rounded-lg border border-[#1a314a] overflow-hidden">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-if="!p.positions.length"><td colspan="5" class="px-4 py-8 text-center text-[#8fa5c6]">暂无持仓记录</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
// 风控
const riskLoading = ref(false); const riskResults = ref<any[]>([]);
async function checkRisks() {
  ...
  } catch(e:any) {} finally { riskLoading.value = false; }
}

// AI 调仓
const adviceLoading = ref(false); const adviceResult = ref<any>(null);
async function getAdvice() {
  ...
  } catch(e:any) {} finally { adviceLoading.value = false; }
}
